...
def softness(edf):
    ...
    for ind,line in enumerate(l1):
        if ind:
            ...
            Z,N,BE,beta2,Q2 = int(ss[0]),int(ss[1]),float(ss[3]),float(ss[4]),float(ss[5])
            d_control[(Z,N)] = (BE,beta2,Q2)

    # from beta_cutoff_035 file
    ...
    if True:
        ...
        "beta2_gs".ljust(15)+"beta2_control".ljust(15)+"Q20_control (b)".ljust(15)+"\n"
        for k in d_control:
            Z,N = k[0],k[1]
            # Z, N, A,
            # E_oct,
            # BE_gs (octupole gs), BE_control (quadrupole gs),
            # beta3_gs, beta2_gs,
            # beta2_control, Q20_control
            ...
            str(d_gs[k][2]).ljust(15)+str(d_gs[k][1]).ljust(15)+\
            str(d_control[k][1]).ljust(15)+str(d_control[k][2]).ljust(15)+"\n"
