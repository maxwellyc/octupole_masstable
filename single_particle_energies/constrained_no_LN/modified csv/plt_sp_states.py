...
def octupole_split():
    ...
    # plot or not
    if True:
        fig, axes = plt.subplots(len(oct_region), 1, sharex = True, sharey = False, figsize = (15,10))
        gs = gridspec.GridSpec(len(oct_region), 1, hspace=0.0, top=0.95, bottom=0.05, left=0.17, right=0.845)
        for ind1,(a,b) in enumerate(oct_region):
            ...
            # n orbitals: n_orb_latex1+"-"+n_orb_latex2
            y_min = min(min(y1),min(y2))
            y_max = max(max(y1),max(y2))
            ax.plot(x0,y1,'b-',marker='s',label="Neutron\t"+n_orb_latex1+"-"+n_orb_latex2)
            # p orbitals: p_orb_latex1+"-"+p_orb_latex2
            ax.plot(x0,y2,'r-',marker='s',label="Proton \t"+p_orb_latex1+"-"+p_orb_latex2)
            #ax.title(region_labels[ind1]+" Region")
            #)
            #ax.set_ylim([-0.5,8])
            ax.text(x=8,y=y_min,s=region_labels[ind1],fontsize=14,fontweight='bold')
            ax.set_xticks(x0)
            ax.set_ylim([y_min-0.1,y_max+0.1])
            y_range = [int(2*y_min+1)/2,int(2*y_max+1)/2]
            
            ax.set_yticks(np.arange(y_range[0],y_range[1],1))
            ax.set_xlim([0.7,9.7])
            ...
            if ind1 == len(oct_region)-1:
                ax.set_xticklabels(edf_xtick,fontsize=18)
            #plt.axes().yaxis.set_minor_locator(ml)
            if ind1 == 3:
                ax.set_ylabel(r"$E_{l,j} - E_{l-3,j-3}$ (MeV)",fontsize=18)
        plt.savefig("plots/splitting/"+"combined_sp_splitting.pdf",format="pdf")
        plt.clf()
