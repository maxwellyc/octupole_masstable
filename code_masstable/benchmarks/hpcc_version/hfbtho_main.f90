...
End Program hfbthoprog
  ...
  Subroutine Main_Program
    ...
    double precision :: beta3       !MCedit 10/30/18
    Logical :: file_exists
    ...
    do iRow = 0,nRows
       ...
       if(mpi_taskid.eq.0) then
          if (iRow .eq. 1) then
            write(127,*) "  Row     Z    N         Q20            Q30           beta2         beta3" !MCedit 10/30/18
          endif
          write(127,*) row_string,Z_chain,N_chain,Q20,Q30,beta_deformation,beta3 !MCedit 11/7/18
       endif
       ...
       beta3_deformation = 10000.D0 * beta3_deformation  !MCedit 10/30/18
       beta3_deformation = nint(beta3_deformation)       !MCedit 10/30/18
       beta3_deformation = beta3_deformation / 10000.D0  !MCedit 10/30/18
#endif
    !-------------------------------------------------------------------
    ...
    if(nrows.gt.0) then
#if(USE_MPI==2)
       call gather_results     !MCedit 9/24/18 comment out gather_result subroutine
       call mpi_barrier(MPI_COMM_WORLD,ierr_mpi) !MCedit 10/01/18
#endif
       call print_mass_table     !MCedit 9/24/18 comment out print_mass_table subroutine
    endif
