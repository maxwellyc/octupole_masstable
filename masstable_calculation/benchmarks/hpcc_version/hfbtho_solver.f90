    ...
    Do
       ...
       If(ILST.Le.0) Then
          ...
          If(iexit>0) Then
             Call start
          Else
             If (VERSION_DATA >= 4) Call gamdel(.false.,.false.) !MCedit-NS
          End If
          If(ierror_flag.Ne.0) Return
          !-------------------------------------------------------------
...
!$OMP PARALLEL SHARED(iw,lout,lfile) PRIVATE(numThreads,idThread)
    ...
    If (idThread .Eq. 0) Then   !MCedit 5/25/18, removed irow.le.1 condition
#elif(DRIP_LINES==1)
...
101                    Format(i4,2f12.6,2(1x,f12.8),2(2x,f7.4),' ',a13)
  ...
  Subroutine initialize_HFBTHO_NAMELIST
    ...
    beta3_deformation = zero    !MCedit 05/28/18
    beta4_deformation = zero    !MCedit 05/28/18
    ! HFBTHO_ITERATIONS
  ...
  Subroutine initialize_HFBTHO_SOLVER
    ...
    !MCcomment The sqrt(117)/(4*pi) cannot be understood (justified). 5/9/18
    !ffdef4=Sqrt(117.0_pr)/(four*pi) !MCcomment 117=6^2+9^2, don't know the meaning of this.
    ffdef4=Sqrt(nine/pi)/two
    ffdef5=Sqrt(nine/(four*pi))/eight
    ffdef6=Sqrt(five*pi)/three
    ffdef65=Sqrt(seven/pi)/two !MCedit 5/9/18
    ffdef66=four*pi/three      !MCedit 5/15/18
    ffdef7=Sqrt(pi)/four
  ...
  !=======================================================================
  Subroutine adjust_basis(q2val,initial_deformations) !MCedit-NS
    Implicit None
    Logical, Intent(IN) :: initial_deformations !MCedit-NS
    Real(pr), Intent(IN) :: q2val
    Real(pr) :: OMEGA0,hbarc,mass_neut,SCALEM !MCedit-NS
    ! Loose fit based on fission of 240Pu, see PRC 90, 054305 (2014)
    If(q2val.Gt.30.0_pr) Then    !MCedit-NS
    ...
    SCALEM=((tz(1)+tz(2))/240.0_pr)**(one/six)
    oscillator_length=hbarc/Sqrt(OMEGA0*mass_neut)*SCALEM
    !MCedit-NS
    basis_deformation=0.05*Sqrt(q2val)
    !MCedit-NS
    If(initial_deformations) Then
        beta2_deformation=basis_deformation
        !beta3_deformation=beta2_deformation/10.0_pr     !MCedit 05/28/18
...
101                Format(' Gauss Laguerre: l='  &
  ...
  Subroutine start
    ...
    Real(pr) :: zb(ngh),rrb(ngl),rb(ngl),rav,rao,vpws,vls,betas,gamma,fac, &
                ...
                !MCedit b3_ws, pleg3 9/5/18   !MCedit-NS zcm,b1_ws,pleg1
    !----------------------------------------------------------------------------
    ...
    If(Abs(b2_0).Gt.1.5_pr) b2_0=1.5_pr ! Avoid crazy initial points (quadrupole deformation) !MCedit-NS
    If(Abs(b3_0).Gt.1.2_pr) b3_0=1.2_pr ! Avoid crazy initial points (octupole deformation), MCedit 05/28/18
    ...
    Do iw=lout,lfile
       ...
       Write(iw,'(a,f14.8)')  '  b3_ws  =',b3_0    !MCedit 05/28/18
    ...
    Do it=itmin,itmax
       ...
       b1_0 = zero                                  !MCedit-NS
       b1_ws = b1_0 * Sqrt(3.0_pr/(4.0_pr*pi))      !MCedit-NS
       b2_ws = b2_0 * Sqrt(5.0_pr/(4.0_pr*pi))
       b3_ws = b3_0 * Sqrt(7.0_pr/(4.0_pr*pi))     !MCedit 05/28/18
    ...
    Else
       ...
       Do il=1,ngl
          Do ih=1,ngh
             r=Sqrt((zb(ih)-zcm)**2+rrb(il))    !MCedit-NS
             If (r.Lt.rc) Then
...
!$OMP End Parallel
  ...
  Subroutine expectpj(lpr)
    ...
    Complex(pr) :: cxn(2),crms(3),cq2(3),cq4(3),xnpj(2),rmspj(3),q2pj(3),q4pj(3)!,q3pj(3),cq3(3) !MCedit cq3(3), q3pj(3), 5/9/18
    ...
    !q3pj   = zero !MCedit q3pj(3), 5/9/18
    !
    ...
    Do k=1,ilpj*ilpj
       ...
       Do ihli = 1,nghl
          ...
          row = whl*rn; cxn(1)=cxn(1)+row; crms(1)=crms(1)+row*rrr; cq2(1)=cq2(1)+row*p2; cq4(1)=cq4(1)+row*p4!; cq3(1)=cq3(1)+row*p3
          row = whl*rp; cxn(2)=cxn(2)+row; crms(2)=crms(2)+row*rrr; cq2(2)=cq2(2)+row*p2; cq4(2)=cq4(2)+row*p4!; cq3(2)=cq3(2)+row*p3
          !MCedit cq3 addition, 5/9/18
       ...
       If(j.Eq.1) Then
          ...
          !q3pj(1)  = q3pj(1)  + pjk(i,1)*cq3(1) !MCedit 5/9/18
       ...
       If(i.Eq.1) Then
          ...
          !q3pj(2)  = q3pj(2)  + pjk(i,2)*cq3(2) !MCedit 5/9/18
    ...
    Do it=itmin,itmax
       ...
       !q3(it) = ffdef65*Real(q3pj(it),Kind=pr) !MCedit 5/9/18
  ...
  Subroutine expect(lpr)
    ...
    If (lpr) Then
       ...
       If(kindhfb.Lt.0) Then
          ...
          !------------------------------------------------
          ! MCcomment 5/9/18 ::
          ! In HFODDv2.73g (User's Guide) pg.65, the author explicitly said for lamda>2, a_lamda_mu = 1.
          ! Thus q3 = r^lambda * Y_lm (standard spherical harmonics) for lambda>2, we shall follow this definition from now on.
          ! This would change how beta3(oct3), beta4(het4) are related to q3, q4 since coeficients of q3, q4 changed.
          ! The relation of beta's and Q's should be derived carefully if one is to change the factors for Q,
          !while beta's are from radius multipole expansion so the definition is unique.
          ! Personally I would prefer the a_lamda_mu for lambda>2 to also follow table in HFODDv2.73g (User's Guide) pg.65
          ! The idea is to make Q_lambda_0 explicitly the numerator part of corresponding spherical harmonics.
          ! However since people have been using this convention for a while, we should stick with HFODDv2.73g (User's Guide) pg.65
          ! MCcomment 5/9/18
          !ffdef4=Sqrt(nine/pi)/two
          !ffdef5=Sqrt(nine/(four*pi))/eight
          !ffdef6=Sqrt(five*pi)/three
          ...
          Do it=itmin,itmax
             ...
             def(it)=Sqrt(pi/5.0_pr)*q2(it)/(rms(it)**2*xn(it))
          End Do
          r212=rms(1)**2; r222=rms(2)**2; r03=r00**3 !MCedit r03
          rms(3)=Sqrt((xn(1)*r212+xn(2)*r222)/amas)
          q2(3)=q2(1)+q2(2)  ! quadrupole moment
          ...
          ! MCcomment 3/21/18 ::
          ! For future record, C_n * Q_n0 = 3/(4*pi) * A * r0^n * beta_n, A is neutron/proton/mass number, C_n is
          !the overall numerical portion of spherical harmonics.
          ! For example: C_2= 1/4 * sqrt(5/pi), C_3 = 1/4 * sqrt(7/pi), C_4 = 3/16 * sqrt(1/pi)
          ! MCcomment 3/21/18
          ! MCcomment 5/9/18 ::
          ! An update to the above relation between beta and Q, since we now adopt a_lambda_mu = 1 for lambda>1, the relation
          !between beta and Q changed, C_3 = 1, C_4 = 1, basically C_lambda := 1 / a_lambda_mu
          ! Q_n0 = 3/(4*pi) * A * r0^n * beta_n , in units of fm
          ! MCcomment 5/9/18
          ...
          bet2(2)=ffdef6*q2(2)/(xn(2)*r02) !x=r0=1.2A^(1/3)
          bet2(3)=ffdef6*q2(3)/(amas*r02)
          oct3(1)=ffdef66*q3(1)/(xn(1)*r03)    !MCedit oct3 for octupole deformation
          oct3(2)=ffdef66*q3(2)/(xn(2)*r03)    !MCedit oct3 for octupole deformation
          oct3(3)=ffdef66*q3(3)/(amas*r03)     !MCedit oct3 for octupole deformation
          het4(1)=ffdef66*q4(1)/(xn(1)*r04)    !MCedit het4 for hexadecapole deformation
          het4(2)=ffdef66*q4(2)/(xn(2)*r04)    !MCedit het4 for hexadecapole deformation
          het4(3)=ffdef66*q4(3)/(amas*r04)     !MCedit het4 for hexadecapole deformation
          xn(3)=xn(1)+xn(2)
          bet=def(3)
          RpRMSsq=0.769_pr
          RnRMSsq=-0.1161_pr   ! J. Phys. G 33, 1 (2006)
          DarwinFoldy=0.033_pr ! Phys. Rev. A 56, 4579 (1997)
          rc = Sqrt(r222 + RpRMSsq + (xn(1)/xn(2))*RnRMSsq + DarwinFoldy)
          ! transitions to barn,barn^2,(10fm)^3,barn^4
          Do i=1,3
             q2(i)=q2(i)/100.0_pr; q4(i)=q4(i)/10000.0_pr
          ...
          ! Nicholas Schunck's multipole moment module is called and outputs are already printed above
          ! expect()<resu()<HFBTHO_DFT_SOLVER, HFBTHO_DFT_SOLVER called in hfbtho_main.f90:l436 (as of 9/7/18)
          ! print_mass_table called in hfbtho_main.f90:l491 (as of 9/7/18)
          ! Here we overwrite qmoment(:,:) array with LN corrected value, MCcomment 9/7/18.

          ! LN correction only edited for q2~q4, MCedit 9/7/18:
          Do it=1,3 ! qmoment of neutron(1), proton(2), total(3)
              qmoment(2,it)=q2(it)
              qmoment(3,it)=q3(it)
              qmoment(4,it)=q4(it)
          End Do
          ...
          Do iw=lout,lfile
             ...
             Write(iw,'(a,3f15.6)') '  q2 deformation,  beta',bet2      !MCedit
             Write(iw,'(a,3f15.6)') '  q3 deformation,  octa',oct3      !MCedit
             Write(iw,'(a,3f15.6)') '  q4 deformation,  heta',het4      !MCedit
             Write(iw,'(a,3f15.6)') '  quadrupole moment[b] ', q2
             Write(iw,'(a,3f15.6)') '  octupole moment[ (10fm)^3] ', q3 !MCedit
             Write(iw,'(a,3f15.6)') '  hexadecapole moment[b^2] ', q4   !MCedit
             Write(iw,'(a,3f15.6)') '================================'
             Write(iw,'(a,3f15.6)')
          End Do
       End If
